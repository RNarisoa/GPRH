...
class DataAccessObject
{
	...
	function getCategorieList()	{
		...
		$this -> _query = "SELECT t.ID_CATEGORIE, t.ID_GROUPE_CATEGORIE, t.CATEGORIE_PROFESSIONNELLE, t.DESCRIPTION_CATEGORIE, t.NOM_GROUPE, h.nb from (SELECT c.ID_CATEGORIE, c.ID_GROUPE_CATEGORIE, c.CATEGORIE_PROFESSIONNELLE, c.DESCRIPTION_CATEGORIE, g.NOM_GROUPE FROM categorie c JOIN groupe_categorie g on c.ID_GROUPE_CATEGORIE = g.ID_GROUPE_CATEGORIE) t LEFT JOIN (SELECT ID_GROUPE, ID_CATEGORIE, ACTIF_LIGNE_GRILLE, count(*) as nb FROM grille WHERE ACTIF_LIGNE_GRILLE = 1 GROUP BY ID_GROUPE, ID_CATEGORIE) h on t.ID_CATEGORIE = h.ID_CATEGORIE and t.ID_GROUPE_CATEGORIE = h.ID_GROUPE";
	...
	function getSalaireBaseList()	{
		$this -> _query = "SELECT s.ID_LIGNE_GRILLE, s.ID_GROUPE, s.ID_CATEGORIE, s.SALAIRE_BASE_MINIMUM, s.SALAIRE_BASE_3, s.SALAIRE_BASE_4, s.SALAIRE_BASE_5, s.ACTIF_LIGNE_GRILLE, s.DATE_PRISE_EFFET_GRILLE, f.NOM_GROUPE, cat.CATEGORIE_PROFESSIONNELLE FROM grille s join groupe_categorie f on s.ID_GROUPE = f.ID_GROUPE_CATEGORIE join categorie cat on cat.ID_CATEGORIE = s.ID_CATEGORIE";
